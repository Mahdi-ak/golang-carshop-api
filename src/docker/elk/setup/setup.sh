...
until curl -s --cacert /usr/share/elasticsearch/config/certs/ca/ca.crt https://es01:9200 | grep -q "missing authentication credentials"; do
  sleep 10
done

echo "Setting kibana_system password"
until curl -s -X POST --cacert /usr/share/elasticsearch/config/certs/ca/ca.crt \
