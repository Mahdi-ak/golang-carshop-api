...
var zeroLoglogLevelMap = map[string]zerolog.Level{
...
func (l *zeroLogger) getLogLevel() zerolog.Level {
	...
	level, exists := zeroLoglogLevelMap[l.cfg.Logger.Level]
	if !exists {
...
func (l *zeroLogger) Init() {
	once.Do(func() {
		...
		file, err := os.OpenFile(l.cfg.Logger.FilePath, os.O_APPEND|os.O_CREATE|os.O_CREATE|os.O_WRONLY, 0666)
		if err != nil {
			panic("could not open log file")
		}
		var logger = zerolog.New(file).With().Timestamp().
			Str("AppName", "MyApp").
